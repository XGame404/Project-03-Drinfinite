...
public static class GameDataManager
{
    ...
    {
        BinarySerializer.Save(playerData, "Drinfinite-testingver4-data.txt");
    ...
    static void LoadPlayerData()
    {
        try
        {
            if (BinarySerializer.HasSaved("Drinfinite-testingver4-data.txt"))
            {
                playerData = BinarySerializer.Load<PlayerData>("Drinfinite-testingver4-data.txt");
                Debug.Log("<color=green>[PlayerData] Loaded.</color>");
            }
            else
            {
                playerData = new PlayerData();
                playerData.coins = 90000;
                playerData.gems = 90000;
